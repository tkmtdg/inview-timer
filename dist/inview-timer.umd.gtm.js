...
'{this.timerID&&(window.clearTimeout(this.timerID),this.dispatch("timer canceled"))}}return class{constructor(t=null){__p'+
'ublicField(this,"avoidIdCollision",(t=>{this.inviewTargets.some((e=>t.id===e.id))&&(this.log(`ID collision detected: "${'+
't.id}"`),t.refreshId(),this.avoidIdCollision(t))})),this.debug=!1,this.observeTargets=[],this.inviewTargets=[],this.invi'+
'ewTargetOptions=null,this.intersectionObserverOptions=null,t&&Object.assign(this,t)}get debug(){return this._debug}set d'+
'ebug(e){this._debug=e,t.debug=this._debug}get observeTargets(){return this._observeTargets}set observeTargets(t){this._o'+
'bserveTargets=t}get inviewTargets(){return this._inviewTargets}set inviewTargets(t){this._inviewTargets=t}get inviewTarg'+
'etOptions(){return this._inviewTargetOptions}set inviewTargetOptions(t){this._inviewTargetOptions=t}get intersectionObse'+
'rverOptions(){return this._intersectionObserverOptions}set intersectionObserverOptions(t){this._intersectionObserverOpti'+
'ons=t}log(...e){t.log(this.constructor.name,...e)}observe(){const t=new IntersectionObserver(((t,e)=>{for(const i of t)f'+
'or(const t of this.inviewTargets)if(t.target===i.target){t.inview=i.isIntersecting;const e=this.inviewTargets.find(((t,e'+
',r)=>t.target===i.target)),r=t.inview?"inview in":"inview out";e.dispatch(r);break}}),this.intersectionObserverOptions);'+
'for(const i of this.observeTargets){const r=new e(__spreadValues({debug:this.debug,target:i},this.inviewTargetOptions));'+
'this.avoidIdCollision(r),this.inviewTargets.push(r),r.target=i,i.addEventListener("inview in",(t=>{this.log(t.type,r.des'+
'cribe),r.setTimer()})),i.addEventListener("inview out",(t=>{this.log(t.type,r.describe),r.clearTimer()})),i.addEventList'+
'ener("timer timedout",(t=>{this.log(t.type,r.describe)})),i.addEventListener("timer canceled",(t=>{this.log(t.type,r.des'+
'cribe)})),t.observe(i)}}}}));');
